...
END_PROMPT = '[END]'
JOINED_PROMPT = 'joined.'
TPT_PROMPT = 'cluster throughput'
OOM_PROMPT = 'shared memory space run out'
DEADLOCK_PROMPT = 'Deadlock'
ABORT_PROMPT = 'Aborted'
SEGMENTATION_FAULT = 'Segmentation fault'
FLOAT_POINT_FAULT = 'Floating point exception'
...
class CMDManager(object):
    ...
    def __init__(self, cluster_ips: list, master_ip: str):
        ...
        SET_HUGEPAGE = 'echo 36864 > /proc/sys/vm/nr_hugepages'
        self.all_execute(SET_HUGEPAGE)
    ...
    @func_set_timeout(500)  # DEBUG
    def all_long_execute(self, command: str, CN_num: int = -1, only_need_tpt: bool=False):
        if CN_num < 0:  # -1 means use all CNs
            CN_num = len(self.__CNs)
        print_OK(f'COMMAND="{command}"')
        print_OK(f'EXECUTE_IPs={self.__cluster_ips[:CN_num]}')
        if not command.endswith('\n'):
            command += '\n'
        for i in range(CN_num):
            self.__clear_shell(self.__shells[i])
        ...
        for i in range(CN_num):
            ...
            while not self.__match_prompt(outs[cli_ip], JOINED_PROMPT if only_need_tpt else END_PROMPT):
                try:
                    msg = self.__shells[i].recv(BUFFER_SIZE).decode()
                    if msg:
                        print(f'[CN {cli_ip} OUTPUT] {msg.strip()}')
                        outs[cli_ip] += msg
                    for prompt in [OOM_PROMPT, DEADLOCK_PROMPT, ABORT_PROMPT, SEGMENTATION_FAULT, FLOAT_POINT_FAULT]:
                        if self.__match_prompt(outs[cli_ip], prompt):
                            raise Exception(prompt)
                except socket.timeout:
                    continue

            if only_need_tpt and self.__match_prompt(outs[cli_ip], TPT_PROMPT):
                break
