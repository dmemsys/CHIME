#
# Fig.17: The contribution of speculative read
#
...
def main(cmd: CMDManager, tp: LogParser):
    plot_data = {
        'methods': methods,
        'X_data': [t[0] * t[1] for t in client_nums],  # client num
        'Y_data': {method: [] for method in methods}        # tpt
    }
    for method in methods:
        project_dir = f"{home_dir}/{'SMART' if method == 'SMART' else 'CHIME'}"
        work_dir = f"{project_dir}/build"
        env_cmd = f"cd {work_dir}"

        # change config
        sed_cmd = (sed_workloads_dir('./workloads.conf', workloads_dir) + " && " +
                    generate_sed_cmd('./include/Common.h', 'SMART' if method == 'SMART' else 'CHIME', 8 if key_type == 'randint' else 32, value_size,
