#
# Fig.3a: Tradeoff of amplification factor and consumed cache size of tree indexes on DM
#
...
def main(cmd: CMDManager, tp: LogParser):
    plot_data = {
        ...
        'Y_data': {method: [] for method in methods}   # consumed cache size
    }
    for method in methods:
        ...
        for ampl_size in ampl_sizes[method]:
            # change config
            span_size = ampl_size if method in ['Sherman', 'SMART'] else ((ampl_size // 2) if method == 'ROLEX' else 64)
            sed_cmd = (sed_workloads_dir('./workloads.conf', workloads_dir) + " && " +
                       generate_sed_cmd('./include/Common.h', method, 8 if key_type == 'randint' else 32, value_size,
                                        ...
                                        {'epsilon': ampl_size // 2, 'neighbor_size': neighbor_size, 'hotspot_buffer_size': hotspot_buffer_size}))
            ...
            print_GOOD(f"[FINISHED POINT] method={method} ampl_size={ampl_size} consumed_cache_size={consumed_cache_size}")
            plot_data['X_data'][method].append(ampl_size)
            plot_data['Y_data'][method].append(consumed_cache_size)
    # save data
