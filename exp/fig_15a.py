#
# Fig.15a: Factor analysis (Sherman => CHIME)
#
...
method_maps = {
    'Sherman': 'Baseline (Sherman)',
    'S +Hopscotch Leaf Node'     : '+Hopscotch Leaf Node',
    'S +Vacancy-Aware Lock'      : '+Vacancy-Aware Lock',
    'S +Metadata Replication'    : '+Metadata Replication',
    'S +Sibling-Based Validation': '+Sibling-Based Validation',
    'S +Speculative Leaf Read'        : '+Speculative Leaf Read (CHIME)'
}
...
def main(cmd: CMDManager, tp: LogParser):
    ...
    for workload, workload_name in workload_names.items():
        ...
        sed_cmd = (sed_workloads_dir('./workloads.conf', workloads_dir) + " && " +
                    generate_sed_cmd('./include/Common.h', 'CHIME', 8 if key_type == 'randint' else 32, value_size,
