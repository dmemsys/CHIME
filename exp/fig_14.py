...
def main(cmd: CMDManager, tp: LogParser):
    plot_data = {
        'methods': methods,
        'bar_groups' : list(map(lambda x : str(x)[:-1], workload_names)),
        'Y_data' : {
            method: {}  # store consumed cache size with various workloads(30/60/90/120), respectively
        ...
        for workload_name in workload_names:
            ...
            while True:
                try:
                    ...
                    consumed_cache_size, consumed_hot_buffer_size = tp.get_cache_statistics(logs)
                    break
