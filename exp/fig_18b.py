#
# Fig.18b: Sensitivity: cache size
#
...
cache_sizes               = exp_params['cache_size']
...
def main(cmd: CMDManager, tp: LogParser):
    ...
    for method in methods:
        ...
        for cache_size in cache_sizes:
            # Rolex
            if method == 'ROLEX' and cache_size < 50:
            ...
            if cache_size == 0:
                cmake_option = cmake_option.replace('-DENABLE_CACHE=on', '-DENABLE_CACHE=off')
            else:
            ...
            while True:
                try:
                    ...
                    tpt, _, _, _, _, _, _ = tp.get_statistics(logs, target_epochs[method], get_avg=True)
                    break
                except (FunctionTimedOut, Exception) as e:
                    print_WARNING(f"Error! Retry... {e}")

            print_GOOD(f"[FINISHED POINT] method={method} cache_size={cache_size}MB tpt={tpt}")
